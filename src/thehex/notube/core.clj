...
(defn handle-video
  [video]
  ;; ensure the channel title is the one we expected
...
(defn parse-comments
  ...
  (handle-comment (first comments)))

(defn handle-comment
  ""
  [comment]
  (let [title (get comment "title")
        text (get (get comment "snippet") "textOriginal")
        author (get comment "authorDisplayName")]
    (log/debugf "Handling comment from video title: %s, commented by user: %s\n" title author)
    (log/debugf "Comment text is: %s\n" text)
    (when (is-spam? text author)
      (log/info "Last comment is spam!")
...
(defn is-spam?
  ...
  (or
   ...
   (some
    ...
    #(condp instance? %
       java.lang.String (str-contains? text %)
       clojure.lang.PersistentVector (every? (fn [b] (str-contains? text b)) %)
       (log/infof "No matching type for entry %s in blacklist.. check that out!" %))
    ;; only in spam for now
    (:spam blacklist))))

;; should i move all common logic in str-contains?
;; think a bit of the impl, algorithm
;; what i want:
;; pass either a string or a vector as the second arg
;; defmulti?


(is-spam? " like = 1" "pewdeipei")
